...
private:
    ...
    //вычисляет данные по формуле указанной в задаче
...
private:
    string inputBin(string text) {
        ...
        while (true) {
            ...
            if (!(result != "" && result[0] != ',' && result[result.size() - 1] != ',' && regex_match(result, regex("[0-1]*?,?[0-1]*")))) {
                SetConsoleTextAttribute(handleConsole, Red);
...
private:
    ...
    //вычисляет данные по формуле в указанной задаче
...
private:
    ...
    // проверяет является ли число сверх простым
...
private:
    ...
    //вычисляет n-ый элемент последовательности 
